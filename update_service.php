...
if(isset($_POST['submit']))
{
  ...
  $price=$_POST['price'];
  $sql="update bitpayments set ServiceName =:servicename,SerDes=:servicedes,ServicePrice=:price  where bitpayments.ID=:eid";
...
<div class="card-body">
  ...
  <form class="forms-sample" method="post" enctype="multipart/form-data" class="form-horizontal">
    <?php
    $eid=$_POST['edit_id'];
    $sql="SELECT * from  bitpayments where bitpayments.ID=:eid";
    $query = $dbh -> prepare($sql);
    $query-> bindParam(':eid', $eid, PDO::PARAM_STR);
    $query->execute();
